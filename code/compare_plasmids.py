...
def create_contigs_dict(plasmids_dict):
	...
	for plasmid in plasmids_dict:
		for contig_id in plasmids_dict[plasmid]:
			if contig_id not in contigs_dict:
				contigs_dict[contig_id] = {}
				contigs_dict[contig_id]['length'] = plasmids_dict[plasmid][contig_id]['length']
				#contigs_dict[contig_id]['sequence'] = plasmids_dict[plasmid][contig_id]['sequence']
				contigs_dict[contig_id]['copies'] = 0
			contigs_dict[contig_id]['copies'] += 1
	return contigs_dict			
...
def rename_contigs(contig, plasmids_dict):
	...
	for plasmid in plasmids_dict:
		if contig in plasmids_dict[plasmid].keys():
			plasmids_dict[plasmid][contig+'_'+str(count)] = plasmids_dict[plasmid].pop(contig)
			count += 1
...
def update_contig_name(plasmids_dict, contig, contig_new):
	for plasmid in plasmids_dict:
		if contig in plasmids_dict[plasmid].keys():
			plasmids_dict[plasmid][contig_new] = plasmids_dict[plasmid].pop(contig)
	return plasmids_dict
...
def rename_by_pmutn(pmutn, left_dict, right_dict, ref_array):
	...
	for item in ref_array:
		...
		count = 0
		while contig_new in set(renamed_contigs):
			count += 1
			idx = idxs[count]
			contig_new = contig+'_'+str(idx) 
		if m < n:
			left_dict = update_contig_name(left_dict, contig, contig_new)
		else:
			right_dict = update_contig_name(right_dict, contig, contig_new)
		renamed_contigs.append(contig_new)
	...
	for left_plasmid in left_dict:
		for right_plasmid in right_dict:
			...
			if left_keys.intersection(right_keys) != set():
				#print((left_plasmid, right_plasmid))
				#print(left_keys.intersection(right_keys))
